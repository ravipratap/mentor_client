...
@Injectable()
export class FieldsOfStudyProvider {
...
//added for debounce support if ionIput is used instead of searchbar in auto complete
  ...
  getResults(keyword:string) {
    ...
    return  new Promise((resolve, reject) => {
      ...
      if(siteConfig && siteConfig.config["fields"] && siteConfig.config["fields"].length > 0) {
        this.fields = siteConfig.config["fields"]
      }  
      const filtered=this.fields.filter(item => item.name.toLowerCase().startsWith(keyword.toLowerCase()) );
      resolve(filtered);
    });
  }
